...
%B=BOUNDARIES(BW) traces the exterior boundaries of objects in the binary image BW.
%B is a p_by_1 cell array,where p is the number of objects in the image.
%Each cell contains a Q_by_2 matrix,each row of which contains the row and column coordinates of a boundary pixel.
%Q is the number of boundary pixels for the corresponding object.
%Object boundaries are traced in the clockwise direction.  
...
%B=BOUNDARIES(BW,CONN,DIR) specifies the direction used for tracing  boundaries.
%DIR should be either 'cw'(trace boundaries clockwise) or 'ccw'(trace boundaries counterclockwise).
...
L=bwlabel(BW,conn);
%The number of objects is the maximum value of L.
%Initialize the cell array B so that each cell initially contains a 0_by_2 matrix. 
numObjects=max(L(:));   
if numObjects>0 
    B={zeros(0,2)};   
...
%Pad label matrix with zeros（用0扩展标签矩阵）.
%This lets us write the boundary_following loop without worrying about going off the edge of the image. 
